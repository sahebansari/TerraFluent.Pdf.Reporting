# Publishing TerraFluent.Pdf.Reporting to NuGet.org

This guide covers everything needed to publish a new version of TerraFluent.Pdf.Reporting as a
...
The first time you push `TerraFluent.Pdf.Reporting`, nuget.org automatically registers
...
3. Give it a name (e.g. `TerraFluent.Pdf.Reporting-GitHub-Actions`).
4. Set **Glob pattern** to `TerraFluent.Pdf.Reporting` (or `*` for all packages).
...
────  ──────────────────────────────────────────────────────────────────
 ...
 5    Create a GitHub Release
      ...
      - Title: TerraFluent.Pdf.Reporting v1.3.0
 ...
 6    The "Publish NuGet" workflow triggers automatically.
      ...
      It will:
        ...
        c. Pack  →  TerraFluent.Pdf.Reporting.1.3.0.nupkg + TerraFluent.Pdf.Reporting.1.3.0.snupkg
        d. Upload packages as a workflow artifact
        e. Push .nupkg to nuget.org
        f. Push .snupkg (symbols) to nuget.org

 7    Verify on nuget.org
      https://www.nuget.org/packages/TerraFluent.Pdf.Reporting
...
```sh
# Build Release
dotnet build src/TerraFluent.Pdf.Reporting/TerraFluent.Pdf.Reporting.csproj -c Release

# Pack to ./artifacts
dotnet pack src/TerraFluent.Pdf.Reporting/TerraFluent.Pdf.Reporting.csproj --no-build -c Release -o ./artifacts

# Inspect contents (rename to .zip on Windows to open in Explorer)
#   Linux/macOS:
unzip -l artifacts/TerraFluent.Pdf.Reporting.*.nupkg

# Test-install from a local feed
dotnet nuget add source ./artifacts --name local-terra
dotnet add <your-test-project>.csproj package TerraFluent.Pdf.Reporting
...
| `TerraFluent.Pdf.Reporting.nuspec` | Generated package manifest |
| `lib/net8.0/TerraFluent.Pdf.Reporting.dll` | Library assembly for .NET 8 |
| `lib/net8.0/TerraFluent.Pdf.Reporting.xml` | IntelliSense XML doc comments |
| `lib/net9.0/TerraFluent.Pdf.Reporting.dll` | Library assembly for .NET 9 |
| `lib/net9.0/TerraFluent.Pdf.Reporting.xml` | IntelliSense XML doc comments |
...
## Key Properties in `TerraFluent.Pdf.Reporting.csproj`
