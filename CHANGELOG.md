...
- **Package renamed from `TerraPDF` to `TerraFluent.Pdf.Reporting`** — the
  library is now the first member of the TerraFluent family of reporting
  packages (upcoming siblings: `TerraFluent.Html.Reporting` and
  `TerraFluent.Docx.Reporting`). PackageId, AssemblyName, RootNamespace, and
  all namespaces changed from `TerraPDF.*` to `TerraFluent.Pdf.Reporting.*`.
- **`Document` entry point renamed to `PdfDocument`** — update call sites from
  `Document.Create(...)` to `PdfDocument.Create(...)`.
- Repository moved to
  [github.com/sahebansari/TerraFluent.Pdf.Reporting](https://github.com/sahebansari/TerraFluent.Pdf.Reporting);
  project website is now [terrafluent.dev](https://terrafluent.dev/).
- Internal PDF serializer class `PdfDocument` renamed to `PdfWriter`
  (no public API impact).

### Fixed
- **Words wider than the available width are now force-broken at character
  level** instead of overflowing the line box. Previously a single unbreakable
  token (e.g. a long URL or dotted identifier) wider than its container was
  drawn past the content area, painting over padding and borders.
...
- Consolidated the two overlapping CI workflows (`ci.yml` + `dotnet.yml`) into
  ...
  never ran).
- Migrated the solution to the XML-based `.slnx` format (`TerraFluent.Pdf.Reporting.slnx`,
  replaces `TerraFluent.Pdf.Reporting.sln`); also fixed a stale mapping that built the library
...
- **Vector Graphics / Canvas API** — `container.Canvas(height, draw)` places a
  ...
  - All coordinates use a **top-left origin** in PDF points, consistent with every
    other TerraFluent.Pdf.Reporting layout API
...
- **Unicode & WinAnsiEncoding showcase sample** (`11_UnicodeShowcase.cs`) — a 5-page
  reference PDF demonstrating TerraFluent.Pdf.Reporting's full WinAnsiEncoding character coverage:
...
### Changed
- Folder `Fluent` renamed to `Core` (`TerraFluent.Pdf.Reporting.Core` namespace).
- Folder `Infrastructure` renamed to `Infra` (`TerraFluent.Pdf.Reporting.Infra` namespace).
...
[Unreleased]: https://github.com/sahebansari/TerraFluent.Pdf.Reporting/compare/v1.2.3...HEAD
[1.2.3]: https://github.com/sahebansari/TerraFluent.Pdf.Reporting/compare/v1.2.2...v1.2.3
[1.2.2]: https://github.com/sahebansari/TerraFluent.Pdf.Reporting/compare/v1.2.1...v1.2.2
[1.2.1]: https://github.com/sahebansari/TerraFluent.Pdf.Reporting/compare/v1.2.0...v1.2.1
[1.2.0]: https://github.com/sahebansari/TerraFluent.Pdf.Reporting/compare/v1.1.0...v1.2.0
[1.1.0]: https://github.com/sahebansari/TerraFluent.Pdf.Reporting/compare/v1.0.0...v1.1.0
[1.0.0]: https://github.com/sahebansari/TerraFluent.Pdf.Reporting/releases/tag/v1.0.0
