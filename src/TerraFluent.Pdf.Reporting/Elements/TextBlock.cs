...
internal sealed class TextBlock : Element
{
    ...
    private static List<WrappedLine> BuildLines(List<TextToken> tokens, double availableWidth)
    {
        ...
        foreach (var token in tokens)
        {
            ...
            // Token does not fit on the current line – flush and start a new one
            ...
            // A single word wider than the whole line can never fit as-is:
            // force-break it at character level so no line ever paints past
            // the available width (and into padding/borders).
            if (tw > availableWidth && !string.IsNullOrWhiteSpace(token.Text))
            {
                var pieces = BreakLongToken(token, availableWidth);
                for (int p = 0; p < pieces.Count - 1; p++)
                    lines.Add(new WrappedLine([pieces[p]], IsLastInParagraph: false));

                current = [pieces[^1]];
                lineW   = TokenWidth(pieces[^1]);
                continue;
            }
    ...
    /// <summary>
    /// Splits a token wider than <paramref name="availableWidth"/> into character-level
    /// chunks that each fit. Always consumes at least one character per chunk so a
    /// pathologically narrow width still terminates (one character per line).
    /// Character widths are additive (no kerning), so incremental accumulation is exact.
    /// </summary>
    private static List<TextToken> BreakLongToken(in TextToken token, double availableWidth)
    {
        var pieces = new List<TextToken>();
        ...
        while (start < text.Length)
        {
            ...
            {
                double cw = TokenWidth(token with { Text = text[end..(end + 1)] });
                if (end > start && w + cw > availableWidth) break;
                w += cw;
                end++;
            }
            pieces.Add(token with { Text = text[start..end] });
            start = end;
        }
        return pieces;
    }
