...
using TerraFluent.Pdf.Reporting.Barcodes.QrCode;
...
public sealed class QrCodeTests
{
    ...
    {
        var qr = QrCodeGenerator.Generate("https://example.com/terrafluent", QrErrorCorrectionLevel.M);
    ...
    {
        byte[] pdf = Build(doc => doc.Page(p => p.Content().QrCode("https://TerraFluent.Pdf.Reporting.example/")));
