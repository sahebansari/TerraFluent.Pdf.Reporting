...
public sealed class HighPriorityFeatureTests
{
    ...
    public void HyperlinkRenders()
    {
        byte[] bytes = Build(c => c.Page(p =>
        {
            p.Size(PageSize.A4);
            p.Content()
             .Hyperlink("https://TerraFluent.Pdf.Reporting.example")
             .Text("Click here");
        }));
        ...
        byte[] bytes = Build(c => c.Page(p =>
        {
            ...
            p.Content().Column(col =>
            {
                ...
                col.Item().Text(t =>
                {
                    ...
                    t.Span("TerraFluent.Pdf.Reporting").Underline().FontColor(Color.Blue.Medium);
                ...
                col.Item()
                   .Hyperlink("https://TerraFluent.Pdf.Reporting.example")
                   .Text("Click to open TerraFluent.Pdf.Reporting website")
                   .Underline()
                ...
                col.Item().Table(table =>
                {
                    ...
                    table.Row(row =>
                    {
                        row.Cell().BorderBottom(0.5, Color.Grey.Lighten2).Padding(5)
                           .Text("TerraFluent.Pdf.Reporting Core");
                        row.Cell().BorderBottom(0.5, Color.Grey.Lighten2).Padding(5)
                           .AlignRight().Text("$199");
                    });
                });
