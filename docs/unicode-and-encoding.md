...
TerraFluent.Pdf.Reporting uses **WinAnsiEncoding** for all built-in Type 1 fonts (Helvetica, Times, Courier
...
ISO-8859-1. Windows-1252 repurposes them for useful typographic glyphs, and TerraFluent.Pdf.Reporting's
...
container.Text("TerraFluent.Pdf.Reporting\u2122");           // trade mark
```
...
These render perfectly in all built-in TerraFluent.Pdf.Reporting fonts:
...
Internally, TerraFluent.Pdf.Reporting converts every non-ASCII character to its WinAnsi byte value and
...
TerraFluent.Pdf.Reporting ships with extended AFM (Adobe Font Metrics) advance-width tables covering the
...
A character renders as `?` when TerraFluent.Pdf.Reporting cannot map it to a WinAnsi byte value. The
...
The `11_UnicodeShowcase.cs` sample in `samples/TerraFluent.Pdf.Reporting.Sample/Samples/` generates a
...
```sh
cd samples/TerraFluent.Pdf.Reporting.Sample
dotnet run
