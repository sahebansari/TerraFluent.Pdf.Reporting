...
internal static class VectorGraphicsShowcase
{
    internal static void Generate(string path)
    {
        ...
        void Caption(TerraFluent.Pdf.Reporting.Infra.IContainer container, string text) =>
            container.PaddingTop(4).AlignCenter()
                     .Text(text).FontSize(9).FontColor(muted).Italic();

        PdfDocument.Create(doc =>
        {
            doc.MetadataTitle("TerraFluent.Pdf.Reporting – Vector Graphics Showcase");
            doc.MetadataAuthor("TerraFluent.Pdf.Reporting Engineering Team");
            doc.MetadataSubject("Demonstrates every Canvas drawing primitive in TerraFluent.Pdf.Reporting");
            ...
            doc.Page(page =>
            {
                    ...
                    col.Item().Background(brand).PaddingVertical(10).PaddingHorizontal(14)
                       .Row(hdr =>
                       {
                           hdr.RelativeItem().AlignMiddle()
                              .Text("TerraFluent.Pdf.Reporting — Vector Graphics Showcase")
                ...
                page.Footer().Column(f =>
                {
                    ...
                    {
                        row.RelativeItem().Text("TerraFluent.Pdf.Reporting — Vector Graphics Showcase")
...
// ---------------------------------------------------------------------------
internal sealed class SectionHeader(string hexColor, string title) : TerraFluent.Pdf.Reporting.Infra.IComponent
{
    public void Compose(TerraFluent.Pdf.Reporting.Infra.IContainer container) =>
