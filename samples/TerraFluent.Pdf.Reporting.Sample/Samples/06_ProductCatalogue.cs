...
internal static class ProductCatalogue
{
    internal static void Generate(string path, string headerImg, string smallImg)
    {
        ...
        {
            ("PDF-100", "TerraFluent PDF Core",           "SDK",        "Text, tables, images",        199.00m),
            ("PDF-200", "TerraFluent PDF Pro",             "SDK",        "All Core + charts, barcodes", 499.00m),
            ("PDF-300", "TerraFluent PDF Enterprise",      "SDK",        "All Pro + FIPS, audit log",  1299.00m),
                    ...
                    col.Item().Background(brand).Padding(10).Row(row =>
                    {
                        row.AutoItem().AlignMiddle().Column(c =>
                        {
                            if (File.Exists(smallImg))
                                ...
                                c.Item().Text("TerraFluent").Bold().FontColor(Color.White);
                        });
                        ...
                        {
                            c.Item().AlignRight().Text("www.terrafluent.example")
                             .FontColor(Color.White).FontSize(9);
                            c.Item().AlignRight().Text("sales@terrafluent.example")
                ...
                page.Content().Padding(30).Column(col =>
                {
                    ...
                    col.Item().Row(row =>
                    {
                        row.RelativeItem(3).Margin(4).Background(light).Padding(12).Column(intro =>
                        {
                            ...
                            intro.Item().PaddingTop(4).Text(
                                "This catalogue lists all TerraFluent.Pdf.Reporting products, integrations, and " +
                    ...
                    col.Item().Row(row =>
                    {
                        row.RelativeItem().Column(c =>
                        {
                            ...
                            c.Item().Text("sales@terrafluent.example").FontColor(teal);
                        ...
                        row.RelativeItem().Column(c =>
                        {
                            ...
                            c.Item().Text("docs.terrafluent.example").FontColor(teal);
                            c.Item().Text("github.com/terrafluent").FontColor(muted);
                        });
                ...
                page.Footer().Background(light).Padding(12).Row(row =>
                {
                    row.ConstantItem(55).AlignMiddle().Column(c =>
                    {
                        if (File.Exists(smallImg))
                            ...
                            c.Item().Text("TerraFluent").Bold().FontColor(brand);
                    });

                    row.RelativeItem().PaddingLeft(10).AlignMiddle()
                       .Text("(c) 2025 TerraFluent Co. Ltd. All rights reserved.")
