...
internal static class BarcodesAndQrShowcase
{
    ...
    internal static void Generate(string path)
    {
        ...
        {
            doc.MetadataTitle("TerraFluent.Pdf.Reporting — Barcodes & QR Codes Showcase");
            ...
            doc.MetadataKeywords("pdf; barcode; code128; qr code; iso 18004; TerraFluent.Pdf.Reporting");
            ...
            doc.Page(page =>
            {
                ...
                page.Header().Column(col =>
                {
                    col.Item().Background(Brand).PaddingVertical(10).PaddingHorizontal(14)
                       .Text("TerraFluent.Pdf.Reporting — Barcodes & QR Codes").Bold().FontSize(15).FontColor(White);
                    col.Item().Canvas(3, _ => { });
                });

                page.Footer().BorderTop(0.5, GridLine).PaddingTop(6).Row(row =>
                {
                    row.RelativeItem().Text("TerraFluent.Pdf.Reporting — Barcodes & QR Codes Showcase").FontSize(8).FontColor(Muted);
                    row.AutoItem().Text(t =>
                    {
                ...
                page.Content().PaddingTop(16).Column(col =>
                {
                    ...
                    col.Item().Background(Light).Border(0.5, GridLine).Padding(14)
                       .Barcode("TERRAFLUENT-2026");
                    CodeBlock(col.Item(), "container.Barcode(\"TERRAFLUENT-2026\");");
                           ...
                           })
                           {
                               row.RelativeItem().Column(c =>
                               {
                                   c.Item().AlignCenter().QrCode(
                                       "https://terrafluent.example/" + level, size: 90, level: level);
                    ...
                    CodeBlock(col.Item(),
                        "container.QrCode(\"https://terrafluent.example/\", size: 90,\n"
                        ...
                        foreach (var (name, sku) in products)
                        {
                            tbl.Row(row =>
                            {
                                row.Cell().Padding(6).AlignCenter()
                                   .QrCode($"https://terrafluent.example/p/{sku}", size: 60);
